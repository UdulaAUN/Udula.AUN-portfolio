    ...
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none isolate bg-slate-950">
      ...
      {starsTiny.map((_, i) => (
        <motion.span
          ...
          transition={{
            duration: 2 + Math.random() * 2.5,   // very quick twinkling
            repeat: Infinity,
            ease: "easeInOut",
            delay: Math.random() * 2,
          }}
      ...
      {starsSmall.map((_, i) => (
        <motion.span
          ...
          animate={{
            y: [0, -35, 0],                    // bigger & faster movement
            opacity: [0.4, 1, 0.4],
            scale: [1, 1.3, 1],
          }}
          transition={{
            duration: 5 + Math.random() * 4,   // much faster
            repeat: Infinity,
            ease: "easeInOut",
            delay: Math.random() * 3,
          }}
      ...
      {starsMedium.map((_, i) => (
        <motion.span
          ...
          transition={{
            duration: 7 + Math.random() * 5,   // snappy medium stars
            repeat: Infinity,
            ease: "easeInOut",
            delay: Math.random() * 4,
          }}
      ...
      {starsLarge.map((_, i) => (
        <motion.span
          ...
          transition={{
            duration: 9 + Math.random() * 6,   // fast & dramatic big stars
            repeat: Infinity,
            ease: "easeInOut",
            delay: Math.random() * 5,
          }}
